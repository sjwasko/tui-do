#!/usr/bin/env bash
# Run the tui-do-api live tests against the dev Vikunja using a scoped API token.
#
...
#
#   1. $TUI_DO_TEST_TOKEN, if already exported;
...
#   ./run-live-tests-token.sh ~/.tui-do-token   # reads it from a file
#   TUI_DO_TEST_TOKEN=tk_... ./run-live-tests-token.sh

set -euo pipefail

DEV_URL="https://dev-box.example.net:8443"
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Belt and braces on top of the guard inside the tests themselves.
case "$DEV_URL" in
  *prod-box*) echo "refusing to run against prod" >&2; exit 1 ;;
esac

if [ -z "${TUI_DO_TEST_TOKEN:-}" ]; then
  if [ $# -ge 1 ]; then
    [ -r "$1" ] || { echo "cannot read token file: $1" >&2; exit 1; }
    TUI_DO_TEST_TOKEN="$(head -n1 "$1" | tr -d '[:space:]')"
  elif [ -t 0 ]; then
    read -rsp "Vikunja API token for ${DEV_URL}: " TUI_DO_TEST_TOKEN
    echo
  else
    echo "No token. Export TUI_DO_TEST_TOKEN, pass a file as \$1, or run this on a terminal." >&2
    exit 1
  fi
fi
[ -n "$TUI_DO_TEST_TOKEN" ] || { echo "token is empty" >&2; exit 1; }
export TUI_DO_TEST_TOKEN
