...
/// Where the spec is fetched from unless `CRIAX_DEV_URL` overrides it.
///
/// Deliberately the *dev* instance. Production is read-only and must not become a
/// routine dependency of the build.
const DEFAULT_SPEC_SOURCE: &str = "https://sw-surface.tail9803a5.ts.net:8443";
...
fn fetch_spec() -> Result<()> {
    let base = std::env::var("CRIAX_DEV_URL").unwrap_or_else(|_| DEFAULT_SPEC_SOURCE.to_string());
