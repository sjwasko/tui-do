...
| **Dev target** | Isolated Vikunja on `sw-surface`, seeded from a prod export. Prod is **read-only, always.** |

## Environment (probed live 2026-08-24)

- **Prod Vikunja:** go-vikunja **v2.5.0**, `https://sw-hp2.tail9803a5.ts.net:8443` → `127.0.0.1:3456`,
...
- **sw-surface:** Surface Laptop 4, i7-1185G7, Arch (kernel 6.19.8), **Docker 29.7.2 installed**, ~11 GB RAM
...
| Target | Where | Note |
|---|---|---|
| **Omarchy Quattro** | this workstation (**Omarchy 4.0.0-1**) + `sw-lenovo` | Primary dev and daily-driver target. `sw-lenovo` is a second Omarchy 4 box — but the KB flags it asleep on a default power policy and unreachable until ~2026-08-30. |
| **Ubuntu 26.04 LTS** | `ubuntu:26.04` container on `sw-surface` | No homelab host runs 26.04 (sw-hp2/sw-hp1/sw-pi are all 24.04.4); the image tag exists and is published. Scripted as `deploy/test-ubuntu.sh` — build + test in-container against the dev Vikunja. Doubles as the glibc-version floor check, since Arch and Ubuntu LTS differ sharply there. |
...
### Phase 0.5 — Dev Vikunja on sw-surface
...
2. **Publish.** `tailscale serve` on `:8443` → `127.0.0.1:3456`, giving
   `https://sw-surface.tail9803a5.ts.net:8443` — same port as prod so only the hostname differs.
...
5. **Prod-write guard, belt and braces.** `tui-do` refuses to run against a URL matching
   `TUI_DO_PROD_DENY` (defaulting to the sw-hp2 host) unless `--i-know-this-is-prod` is passed, and
...
6. **KB update — ask first.** `common-hosts.md` has an explicit agent write policy: *an agent may edit it,
   ...
   **proposal step, not an automatic one**: draft the diff replacing sw-surface's `DECOMMISSION PENDING`
...
- **Dev instance:** Phase 0.5 is done when `curl https://sw-surface.tail9803a5.ts.net:8443/api/v1/info`
