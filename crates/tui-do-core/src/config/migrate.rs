...
//!
//! tui-do's schema is a clean break, so this is a one-way translation rather than a
...
//! *someone else's* file — it may contain keys tui-do has never heard of, or keys cria
...
pub struct Migration {
    ...
    ///
    /// Shown to the user after `tui-do migrate`, because a silent partial import is how
...
impl CriaConfig {
    fn into_migration(self) -> Migration {
        ...
        // cria's api_url usually already ends in /api/v1; tui-do's client accepts either,
...
/// Translate cria's columns, dropping the disabled ones and any tui-do has no field for.
fn convert_columns(columns: Vec<CriaTableColumn>, notes: &mut Vec<String>) -> Vec<ColumnSpec> {
    columns
        .into_iter()
        // cria carries disabled columns in the list; tui-do expresses that by leaving
        ...
        .filter_map(|c| match convert_column_type(&c.column_type) {
            ...
            None => {
                notes.push(format!(
                    "dropped column {:?}, which tui-do does not have",
                    c.column_type
                ));
                None
            }
...
/// Map cria's column-type spelling onto tui-do's.
...
fn convert_quick_action(action: CriaQuickAction, notes: &mut Vec<String>) -> Option<QuickAction> {
    ...
    let kind = match action.action.to_ascii_lowercase().as_str() {
        ...
        other => {
            notes.push(format!(
                "dropped quick action {key:?}: tui-do has no {other:?} action"
            ));
            return None;
        }
    };
...
";
    ...
    fn a_realistic_cria_config_imports_cleanly() {
        ...
        // The /api/v1 suffix comes off: tui-do stores the server, not the API root.
    ...
    #[test]
    fn the_imported_config_is_one_tui_do_will_actually_load() {
    ...
    fn unknown_keys_do_not_stop_the_import() {
        // cria's config may carry keys from a version tui-do never saw. Refusing the whole
...
";
    ...
    fn both_credential_forms_resolve_to_the_file() {
        // tui-do refuses both at once, so the import has to pick -- and the file is the
        // better half to keep.
        let yaml = "api_url: https://x\napi_key: tk_inline\napi_key_file: ~/.token\n";
        let migration = from_cria_yaml(yaml).unwrap();
