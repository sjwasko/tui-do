...
/// name. A config pointing at `https://100.76.214.113:8443` reached exactly the same server
...
const PROD_ADDRS: &[&str] = &["100.76.214.113", "192.168.1.79"];
...
/// string was wrong in both directions: it missed `https://100.76.214.113:8443`, which is
/// prod, and it would have refused `https://dev.example.com/?note=sw-hp2`, which is not.
///
/// A name matches when it *is* a production host or when its first label is one, so both
/// `sw-hp2` and `sw-hp2.tail9803a5.ts.net` are caught while `sw-hp2-notreally.example.com`
...
mod tests {
    ...
    fn every_way_of_naming_production_is_refused() {
        for url in [
            ...
            "https://sw-hp2.tail9803a5.ts.net./",     // fully-qualified trailing dot
            "https://sw-hp2.tail9803a5.ts.net",       // no port
            "http://sw-hp2:8443",                     // short name
            "https://100.76.214.113:8443",            // tailnet address -- BUG-6
            "https://192.168.1.79:8443",              // LAN address
        ] {
            assert!(
    ...
    fn everything_that_is_not_production_still_starts() {
        for url in [
            "https://sw-surface.tail9803a5.ts.net:8443", // the dev instance
            "https://100.71.225.24:8443",                // dev by address
            "http://localhost:3456",
            "https://vikunja.example.com",
            // The old check substring-matched the whole URL, so this was refused for a
            // word in its query string.
            "https://dev.example.com/?note=sw-hp2",
            // A different machine whose name merely begins the same way.
            "https://sw-hp2-notreally.example.com",
        ] {
            assert!(
