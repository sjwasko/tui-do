...
//! response directly. That is what makes criax start instantly, work offline, and never
...
//!
//! The store speaks [`criax_api::models::Task`] rather than a parallel domain struct.
...
/// buy concurrency SQLite cannot use for writes anyway, and the reads criax makes are
...
impl Store {
    ...
    /// [`CoreError::Config`] if the database was written by a newer criax.
...
mod tests {
    ...
    async fn opening_creates_the_file_and_its_parent_directory() {
        let dir = std::env::temp_dir().join(format!("criax-store-test-{}", std::process::id()));
        let path = dir.join("nested").join("criax.db");
