...
//!
//! criax reads `~/.config/criax/config.yaml` (or `$XDG_CONFIG_HOME/criax/config.yaml`).
//! The schema is a clean break from cria's, and `criax migrate` imports the old one —
//! see [`migrate`].
//!
...
impl Config {
    /// The path criax will read, given an optional explicit override.
    ///
    /// Precedence: the `--config` argument, then `CRIAX_CONFIG`, then the XDG location.
    ///
    /// # Errors
    ...
    /// The directory criax keeps its config in.
    ///
    /// # Errors
    ...
    /// key criax does not recognise. Each carries the path and the underlying reason,
    ...
    /// An example config, for `criax init` and for the README.
    #[must_use]
    pub fn example() -> Self {
        Self {
            server: ServerConfig {
                ...
                token: None,
                token_file: Some(PathBuf::from("~/.config/criax/token")),
                username: None,
            },
...
mod tests {
    ...
    fn a_relative_token_file_resolves_against_the_config_directory() {
        let resolved = resolve_relative(
            Path::new("token"),
            Path::new("/home/someone/.config/criax/config.yaml"),
        );
        assert_eq!(resolved, PathBuf::from("/home/someone/.config/criax/token"));
    }

    #[test]
    fn an_absolute_token_file_is_left_alone() {
        let resolved = resolve_relative(Path::new("/etc/criax/token"), Path::new("/tmp/c.yaml"));
        assert_eq!(resolved, PathBuf::from("/etc/criax/token"));
    }
