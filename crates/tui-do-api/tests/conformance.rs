...
//!
//! - every path template in [`tui_do_api::endpoints`] must exist in the spec, so an
...
use tui_do_api::endpoints;
use tui_do_api::models::{
...
fn the_renamed_tasks_endpoint_is_not_resurrected() {
    // The specific regression tui-do exists to prevent: cria hardcoded /tasks/all, which
...
fn check_model<T: Serialize + Default>(spec: &Value, definition: &str) {
    ...
    assert!(
        unknown.is_empty(),
        "{definition}: these fields are sent or expected by tui-do but are not in the spec: \
...
fn pagination_headers_are_documented_as_expected() {
    // The pagination contract tui-do relies on. cria ignored these headers, requested
